  ...
  const faq = [
    {
      ...
      answer: "Можно одновременно запустить не более двух алгоритмов. Вы можете запустить спотовый и фьючерсный алгоритмы по отдельности.",
    },
    {
      question: "Как пополнить депозит для алгоритма, который уже создан?",
      answer: "Если вы хотите изменить сумму депозита для уже созданного алгоритма, вам необходимо перейти в раздел редактирования алгоритма, внести изменения, сохранить их и запустить алгоритм.",
    },
    {
      question: "Если удалить алгоритм, будут ли закрыты открытые сделки?",
      answer: "Если вы удалите алгоритм, все сделки будут закрыты по текущей цене с помощью маркет-ордера. Однако если вы хотите только остановить алгоритм, чтобы предотвратить открытие новых сделок, но позволить текущим сделкам закрыться в соответствии с условиями алгоритма, вы можете нажать кнопку «Остановить». После этого алгоритм не будет открывать новые сделки, но закроет текущие согласно условиям алгоритма.",
    },
    {
      question: "Можно изменить уже созданный алгоритм?",
      answer: "Вы можете изменять параметры алгоритма и переключать режимы настройки с автоматического на ручной, нажав на кнопку «Редактировать» для действующего алгоритма.",
    },
    {
      question:
        "Где я могу увидеть результаты торговли алгоритма за предыдущие периоды?",
      answer: "Вы можете посмотреть результаты бэктестирования и подробно изучить описание алгоритма, нажав кнопку «Подробнее» в правом верхнем углу интересующего вас алгоритма.",
    },
    {
      question: "Как долго алгоритм может не совершать сделки?",
      answer: "Согласно статистике, алгоритм обычно совершает сделки каждый день, но это зависит от рыночной ситуации. Поэтому, если сделок нет более одного дня, вы можете обратиться в службу технической поддержки или следить за состоянием текущего алгоритма.",
    },
    {
      question:
        "Как понять, что алгоритм функционирует и запуск прошёл успешно?",
      answer: "После создания алгоритма вы можете проверить его статус. Если статус указан как «запускается», это означает, что алгоритм всё ещё находится в процессе создания. Когда статус станет «запущен», это будет означать, что алгоритм успешно запущен и работает корректно.",
    },
  ];
  const faqEn = [
    {
      ...
      <div className="pages_wrapper investments_page">
        ...
        <div className="algo_cards">
          ...
          <div className="algo_card">
            ...
            <div className="algo_card_desc">
              <p>
              Реагирует на импульсное движение цены и подбирает оптимальные точки входа в зависимости от волатильности рынка.
              </p>
            </div>
            <div className="algo_card_graph">
              <LongShortGraph />
